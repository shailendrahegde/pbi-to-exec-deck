...
Converts Power BI dashboard exports (.pptx, .pdf, .pbip) into executive-ready 16:9 PPTX presentations using Copilot's vision and analytical capabilities. No API key needed — uses current Copilot Chat session.

## Workflow

```bash
python convert_dashboard.py "<path>" --prepare --assistant copilot  # Step 1: Extract
python convert_dashboard.py --build --output "<output>.pptx"        # Step 3: Build
python convert_dashboard.py --build --output "<output>.pptx" --vector-charts  # Step 3: Vector charts
python convert_dashboard.py --verify                                 # Validate insights.json
```

**Three steps:**
1. **Extract (auto):** Images → `temp/slide_N.png`, metadata → `temp/analysis_request.json` (with `text_layer`, `text_metrics`, `text_key_phrases`).
2. **Analyze (you):** Read `temp/analysis_request.json` and each `temp/slide_N.png` → generate insights → write `temp/insights.json`.
3. **Build (auto):** Renders slides, validates against Constitution.
...
Use `--vector-charts` when: exact DAX data available, screenshots are low quality. Keep default when: PBI visuals are clean, or complex chart types (dual-axis, heatmaps) are hard to reconstruct.
...
Each slide in `analysis_request.json` includes enriched text fields (Copilot path only):
- `text_layer` — raw extracted text (visual titles, labels, chart types)
- `text_metrics` — extracted numeric values with context
- `text_key_phrases` — key phrases for slide context

**Rules:** Cross-reference `text_layer` with the image for chart titles and axis labels. Use `text_metrics` values when confirmed visible in the image. **Always verify against the image — it is the source of truth.** Never trust `text_metrics[].context` alone; OCR flattens spatial groupings.
...
**Missing data:** If a page has no numbers → headline = "Insufficient data for analysis", list what data is needed, `"numbers_used": []`. Do NOT generate vague insights.
...
// scatter
{"type": "scatter", "title": "Usage vs Satisfaction", "x_label": "Weekly Actions", "y_label": "Score",
 "series": [{"name": "Team A", "x": 45, "y": 8.2}, {"name": "Team B", "x": 32, "y": 7.5, "highlight": true}]}
...
**Example:** Page shows `180 Agent Users | 3.60 Agent Sessions/User | 2,585 Chat Users | 3.3 Chat Sessions/User`. OCR stream flattens to `180 · 3.60 · 2,585 · 3.3`. Read the label below each number to confirm pairing before assigning values.
...
- [ ] Filter selections identified (black/highlighted = active)
...
Always verify number-to-label pairing against the actual image. Do NOT trust `text_metrics[].context` — OCR flattens spatial groupings. Example: seeing `151` near "AI Skills" does NOT mean "151 people have AI Skills" — verify which label is above/below the number in the image.
...
2. Execute DAX via `powerbi-modeling` MCP for each page's queries
...
**Use `measure_dax` to understand how each KPI is calculated:** `DATESYTD()` → year-to-date; `DIVIDE()` with `IF()` → zero-division guard; `CALCULATE()` with filter → note what filter applies.

Reference measure names in insights: "47.3% completion rate ([Project Completion Rate])".
...
- **Answer "So What?":** Every insight implies an action. "Client Finance pattern ready to replicate" ✅ — "Client Finance has 217 prompts/user" ❌
- **Visibility rule:** Only mention platforms/teams/features visible on this specific page.
- **VP forward test:** "Would a VP forward this bullet to their boss?" If no → rewrite.

### DON'T
- Critical/extreme language: "crisis", "catastrophically" → use "opportunity", "challenge"
